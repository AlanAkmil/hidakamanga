...
class AnimekuindoScraper {
  ...
  async detail(slug) {
    const url = slug.startsWith('http') ? slug : this.baseUrl + '/anime/' + slug.replace(/^\/+/, '');
    const html = await fetchHTML(url);
    const $ = cheerio.load(html);
  ...
  async episode(url) {
    ...
    $('.player-embed iframe').each((i, el) => {
      ...
      if (src) { iframeUrl = src; videoUrl = src; return false; }
    });
    ...
    if (!iframeUrl) {
      $('select.mirror option').each((i, el) => {
        ...
        if (value) {
          ...
          if (decoded) { iframeUrl = decoded; videoUrl = decoded; return false; }
        }
      });
    }
